...
namespace PCS {
    public class PCSImpl : GStore.PCS.PCSBase {
        ...
        public override Task<ServerResponse> Server(ServerRequest request, ServerCallContext context) {
            Console.WriteLine(String.Format("[{3}] Request to launch server with id '{0}', min delay of {1}ms and max delay of {2}ms", request.Id, request.MinDelay, request.MaxDelay, DateTime.Now.ToString()));
